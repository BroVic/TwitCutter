...
//    on storage and stream objects (that contain user-defined data), arranged in 
//    hierarchical tree structure to enable the location of the different streams
...
struct CFHeader
{
  CFHeader();
  ~CFHeader();
  std::vector<unsigned long> copy_difat(std::istream&);
